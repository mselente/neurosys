...
Base URL
Code
https://workflows.neurosys.com/v1
...
trace_id (string) Optional Trace ID. Used for integration with existing business trace components to achieve end-to-end distributed tracing. If not provided, the system will automatically generate a trace_id. Supports the following three ways to pass, in order of priority:
...
POST
/workflows/run
curl -X POST 'https://workflows.neurosys.com/v1/workflows/run' \
...
def upload_file(file_path, user):
    upload_url = "https://dify.ai/v1/files/upload"
    headers = {
...
def run_workflow(file_id, user, response_mode="blocking"):
    workflow_url = "https://dify.ai/v1/workflows/run"
    headers = {
...
POST
/workflows/:workflow_id/run
curl -X POST 'https://workflows.neurosys.com/v1/workflows/{workflow_id}/run' \
--header 'Authorization: Bearer {api_key}' \
--header 'Content-Type: application/json' \
--data-raw '{
  "inputs": {},
...
GET
/workflows/run/:workflow_run_id
curl -X GET 'https://workflows.neurosys.com/v1/workflows/run/:workflow_run_id' \
...
POST
/workflows/tasks/:task_id/stop
curl -X POST 'https://workflows.neurosys.com/v1/workflows/tasks/:task_id/stop' \
-H 'Authorization: Bearer {api_key}' \
-H 'Content-Type: application/json' \
...
POST
/files/upload
curl -X POST 'https://workflows.neurosys.com/v1/files/upload' \
--header 'Authorization: Bearer {api_key}' \
...
GET
/workflows/logs
curl -X GET 'https://workflows.neurosys.com/v1/workflows/logs'\
--header 'Authorization: Bearer {api_key}'
...
GET
/info
curl -X GET 'https://workflows.neurosys.com/v1/info' \
-H 'Authorization: Bearer {api_key}'
...
document (object) Document settings Currently only supports document types: txt, md, markdown, pdf, html, xlsx, xls, docx, csv, eml, msg, pptx, ppt, xml, epub.
...
transfer_methods (array[string]) List of transfer methods: remote_url, local_file. Must choose one.
audio (object) Audio settings Currently only supports audio types: mp3, m4a, wav, webm, amr.
enabled (bool) Whether it is enabled
number_limits (int) Audio number limit, default is 3
transfer_methods (array[string]) List of transfer methods: remote_url, local_file. Must choose one.
video (object) Video settings Currently only supports video types: mp4, mov, mpeg, mpga.
enabled (bool) Whether it is enabled
...
GET
/parameters
 curl -X GET 'https://workflows.neurosys.com/v1/parameters'
...
GET
/site
curl -X GET 'https://workflows.neurosys.com/v1/site' \
-H 'Authorization: Bearer {api_key}'
